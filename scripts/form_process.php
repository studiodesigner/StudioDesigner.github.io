...
function process_form($form) {
	if ($_SERVER['REQUEST_METHOD'] != 'POST')
		die(get_form_error_response('不明的伺服器要求方式'));

	if (formthrottle_too_many_submissions($_SERVER['REMOTE_ADDR']))
		die(get_form_error_response('此 IP 最近有太多送出記錄'));
	...
	switch($properties['type']) {
		...
		case 'recaptcha':
			if (!array_key_exists('recaptcha', $form) || !array_key_exists('private_key', $form['recaptcha']) || empty($form['recaptcha']['private_key']))
				die(get_form_error_response('無效的 reCAPTCHA 私密金鑰。'));
		...
		default:
			die(get_form_error_response(sprintf('不明的欄位類型「%s」。', $properties['type'])));
	}
}
...
function email_form_submission($form) {
	...
	if(!$sent)
		die(get_form_error_response('傳送電子郵件失敗'));
...
function get_email_body($subject, $heading, $fields) {
	...
	$message .= '</table>';
	$message .= '<br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">表單由此網站送出: ' . encode_for_form($_SERVER['SERVER_NAME']) . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">訪客 IP 位址: ' . encode_for_form($_SERVER['REMOTE_ADDR']) . '</div>';
	$message .= '</body></html>';
...
function get_form_field_value($field, $properties) {
	...
	switch($properties['type']) {
		case 'checkbox':
			return (($value == '1' || $value == 'true') ? '已核取' : '取消核取');
